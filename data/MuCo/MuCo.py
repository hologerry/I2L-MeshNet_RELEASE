...
class MuCo(torch.utils.data.Dataset):
    ...
    def get_smpl_coord(self, smpl_param, cam_param, do_flip, img_shape):
        ...
        smpl_trans = torch.FloatTensor(trans).view(
            1, 3
        )  # translation vector from smpl coordinate to muco world coordinate
    ...
    def get_fitting_error(self, muco_joint, smpl_mesh, do_flip):
        ...
        if do_flip:
            ...
            for pair in self.muco_flip_pairs:
                muco_joint[pair[0], :], muco_joint[pair[1], :] = (
                    muco_joint[pair[1], :].copy(),
                    muco_joint[pair[0], :].copy(),
                )
    ...
    def __getitem__(self, idx):
        ...
        if do_flip:
            ...
            for pair in self.muco_flip_pairs:
                muco_joint_img[pair[0], :], muco_joint_img[pair[1], :] = (
                    muco_joint_img[pair[1], :].copy(),
                    muco_joint_img[pair[0], :].copy(),
                )
                muco_joint_cam[pair[0], :], muco_joint_cam[pair[1], :] = (
                    muco_joint_cam[pair[1], :].copy(),
                    muco_joint_cam[pair[0], :].copy(),
                )
                muco_joint_valid[pair[0], :], muco_joint_valid[pair[1], :] = (
                    muco_joint_valid[pair[1], :].copy(),
        ...
        muco_joint_img[:, 2] = (
            (muco_joint_img[:, 2] / (cfg.bbox_3d_size * 1000 / 2) + 1) / 2.0 * cfg.output_hm_shape[0]
        )  # change cfg.bbox_3d_size from meter to milimeter

        # check truncation
        muco_joint_trunc = muco_joint_valid * (
            (muco_joint_img[:, 0] >= 0)
            * (muco_joint_img[:, 0] < cfg.output_hm_shape[2])
            * (muco_joint_img[:, 1] >= 0)
            * (muco_joint_img[:, 1] < cfg.output_hm_shape[1])
            * (muco_joint_img[:, 2] >= 0)
            * (muco_joint_img[:, 2] < cfg.output_hm_shape[0])
        ...
        # muco coordinate
        muco_joint_cam = (
            np.dot(rot_aug_mat, muco_joint_cam.transpose(1, 0)).transpose(1, 0) / 1000
        )  # milimeter to meter
        # parameter

...
from main.transforms import cam2pixel, pixel2cam, rigid_align
...
class PW3D(torch.utils.data.Dataset):
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            mesh_out_img[:, 2] = (mesh_out_img[:, 2] / cfg.output_hm_shape[0] * 2.0 - 1) * (cfg.bbox_3d_size / 2)
            ...
            eval_result["mpjpe_lixel"].append(
                np.sqrt(np.sum((pose_coord_out_h36m - pose_coord_gt_h36m) ** 2, 1)).mean() * 1000
            )  # meter -> milimeter
            eval_result["pa_mpjpe_lixel"].append(
                np.sqrt(np.sum((pose_coord_out_h36m_aligned - pose_coord_gt_h36m) ** 2, 1)).mean() * 1000
            )  # meter -> milimeter

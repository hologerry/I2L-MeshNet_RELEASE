...
from main.preprocessing import augmentation, load_img, process_bbox
from main.smpl import SMPL
from main.transforms import cam2pixel, pixel2cam, transform_joint_to_other_db, world2cam
...
class MSCOCO(torch.utils.data.Dataset):
    ...
    def get_smpl_coord(self, smpl_param, cam_param, do_flip, img_shape):
        ...
        if do_flip:  # avg of old and new root joint should be image center.
            ...
            flip_trans_x = (
                2 * (((img_shape[1] - 1) / 2.0 - princpt[0]) / focal[0] * (smpl_joint_coord[self.root_joint_idx, 2]))
        ...
        if self.data_split == "train":
            ...
            if do_flip:
                ...
                for pair in self.coco_flip_pairs:
                    coco_joint_img[pair[0], :], coco_joint_img[pair[1], :] = (
                        coco_joint_img[pair[1], :].copy(),
                        coco_joint_img[pair[0], :].copy(),
                    )
                    coco_joint_valid[pair[0], :], coco_joint_valid[pair[1], :] = (
                        coco_joint_valid[pair[1], :].copy(),
            ...
            coco_joint_trunc = coco_joint_valid * (
                (coco_joint_img[:, 0] >= 0)
                * (coco_joint_img[:, 0] < cfg.output_hm_shape[2])
                * (coco_joint_img[:, 1] >= 0)
                * (coco_joint_img[:, 1] < cfg.output_hm_shape[1])
            ...
            if smplify_result is not None:
                ...
                smpl_coord_img[:, 2] = (
                    (smpl_coord_img[:, 2] / (cfg.bbox_3d_size / 2) + 1) / 2.0 * cfg.output_hm_shape[0]
                )

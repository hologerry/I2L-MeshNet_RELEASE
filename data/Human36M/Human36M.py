...
from main.vis import save_obj, vis_keypoints, vis_mesh
...
class Human36M(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            joint_world = np.array(
                joints[str(subject)][str(action_idx)][str(subaction_idx)][str(frame_idx)], dtype=np.float32
            )
    ...
    def get_smpl_coord(self, smpl_param, cam_param, do_flip, img_shape):
        ...
        if do_flip:
            for pair in self.flip_pairs:
                if pair[0] < len(smpl_pose) and pair[1] < len(
                    ...
                    smpl_pose[pair[0], :], smpl_pose[pair[1], :] = (
                        smpl_pose[pair[1], :].clone(),
        ...
        if do_flip:  # avg of old and new root joint should be image center.
            ...
            flip_trans_x = (
                2
                * (
                    ((img_shape[1] - 1) / 2.0 - princpt[0])
                    / focal[0]
                    * (smpl_joint_coord[self.root_joint_idx, 2] * 1000)
                )
                / 1000
        ...
        smpl_shape[(smpl_shape.abs() > 3).any(dim=1)] = 0.0
    ...
    def get_fitting_error(self, h36m_joint, smpl_mesh, do_flip):
        ...
        if do_flip:
            ...
            for pair in self.h36m_flip_pairs:
                h36m_joint[pair[0], :], h36m_joint[pair[1], :] = (
                    h36m_joint[pair[1], :].copy(),
                    h36m_joint[pair[0], :].copy(),
                )

        h36m_from_smpl = np.dot(self.h36m_joint_regressor, smpl_mesh)
        h36m_from_smpl = (
            h36m_from_smpl - np.mean(h36m_from_smpl, 0)[None, :] + np.mean(h36m_joint, 0)[None, :]
    ...
    def __getitem__(self, idx):
        ...
        img_path, img_shape, bbox, smpl_param, cam_param = (
            data["img_path"],
            data["img_shape"],
            data["bbox"],
            data["smpl_param"],
        ...
        if self.data_split == "train":
            ...
            if do_flip:
                ...
                for pair in self.h36m_flip_pairs:
                    h36m_joint_img[pair[0], :], h36m_joint_img[pair[1], :] = (
                        h36m_joint_img[pair[1], :].copy(),
                        h36m_joint_img[pair[0], :].copy(),
                    )
                    h36m_joint_cam[pair[0], :], h36m_joint_cam[pair[1], :] = (
                        h36m_joint_cam[pair[1], :].copy(),
                        h36m_joint_cam[pair[0], :].copy(),
                    )
                    h36m_joint_valid[pair[0], :], h36m_joint_valid[pair[1], :] = (
                        h36m_joint_valid[pair[1], :].copy(),
            ...
            h36m_joint_img[:, 2] = (
                (h36m_joint_img[:, 2] / (cfg.bbox_3d_size * 1000 / 2) + 1) / 2.0 * cfg.output_hm_shape[0]
            )  # change cfg.bbox_3d_size from meter to milimeter

            # check truncation
            h36m_joint_trunc = h36m_joint_valid * (
                (h36m_joint_img[:, 0] >= 0)
                * (h36m_joint_img[:, 0] < cfg.output_hm_shape[2])
                * (h36m_joint_img[:, 1] >= 0)
                * (h36m_joint_img[:, 1] < cfg.output_hm_shape[1])
                * (h36m_joint_img[:, 2] >= 0)
                * (h36m_joint_img[:, 2] < cfg.output_hm_shape[0])
            ...
            if smpl_param is not None:
                ...
                smpl_coord_img[:, 2] = (
                    (smpl_coord_img[:, 2] / (cfg.bbox_3d_size * 1000 / 2) + 1) / 2.0 * cfg.output_hm_shape[0]
                )  # change cfg.bbox_3d_size from meter to milimeter
                ...
                smpl_trunc = (
                    (
                        (smpl_coord_img[:, 0] >= 0)
                        * (smpl_coord_img[:, 0] < cfg.output_hm_shape[2])
                        * (smpl_coord_img[:, 1] >= 0)
                        * (smpl_coord_img[:, 1] < cfg.output_hm_shape[1])
                        * (smpl_coord_img[:, 2] >= 0)
                        * (smpl_coord_img[:, 2] < cfg.output_hm_shape[0])
                    )
                    .reshape(-1, 1)
                    .astype(np.float32)
                )
            ...
            # h36m coordinate
            h36m_joint_cam = (
                np.dot(rot_aug_mat, h36m_joint_cam.transpose(1, 0)).transpose(1, 0) / 1000
            )  # millimeter to meter
            # parameter
            ...
            smpl_joint_cam = (
                np.dot(rot_aug_mat, smpl_joint_cam.transpose(1, 0)).transpose(1, 0) / 1000
            )  # milimeter to meter
            ...
            meta_info = {
                "orig_joint_valid": h36m_joint_valid,
                "orig_joint_trunc": h36m_joint_trunc,
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            mesh_out_img[:, 2] = (mesh_out_img[:, 2] / cfg.output_hm_shape[0] * 2.0 - 1) * (
                cfg.bbox_3d_size * 1000 / 2
            )
            ...
            pose_coord_gt_h36m = (
                pose_coord_gt_h36m - pose_coord_gt_h36m[self.h36m_root_joint_idx, None]
            )  # root-relative
            ...
            pose_coord_out_h36m = (
                pose_coord_out_h36m - pose_coord_out_h36m[self.h36m_root_joint_idx, None]
            )  # root-relative
            ...
            eval_result["pa_mpjpe_lixel"].append(
                np.sqrt(np.sum((pose_coord_out_h36m_aligned - pose_coord_gt_h36m) ** 2, 1)).mean()
            )
            ...
            if cfg.stage == "param":
                ...
                eval_result["pa_mpjpe_param"].append(
                    np.sqrt(np.sum((pose_coord_out_h36m_aligned - pose_coord_gt_h36m) ** 2, 1)).mean()
                )

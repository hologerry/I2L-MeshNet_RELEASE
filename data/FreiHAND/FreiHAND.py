...
class FreiHAND(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path, img_shape, bbox, joint_cam, cam_param, mano_param = (
            data["img_path"],
            data["img_shape"],
            data["bbox"],
            data["joint_cam"],
        ...
        if self.data_split == "train":
            ...
            mano_coord_img[:, 2] = (mano_coord_img[:, 2] / (cfg.bbox_3d_size / 2) + 1) / 2.0 * cfg.output_hm_shape[0]

            # check truncation
            mano_trunc = (
                (
                    (mano_coord_img[:, 0] >= 0)
                    * (mano_coord_img[:, 0] < cfg.output_hm_shape[2])
                    * (mano_coord_img[:, 1] >= 0)
                    * (mano_coord_img[:, 1] < cfg.output_hm_shape[1])
                    * (mano_coord_img[:, 2] >= 0)
                    * (mano_coord_img[:, 2] < cfg.output_hm_shape[0])
                )
                .reshape(-1, 1)
                .astype(np.float32)
            )
            ...
            # mano coordinate
            mano_joint_cam = (
                mano_joint_cam - mano_joint_cam[self.root_joint_idx, None]
            ) * 1000  # root-relative, meter to millimeter

...
smpl_layer = SMPL_Layer(gender="neutral", model_root="common/utils/smplpytorch/smplpytorch/native/models")

root_path = "/path/to/3DPW/"
img_id = 0
annot_id = 0
    ...
    for annot in tqdm(annot_list):
        ...
        for iid in tqdm(range(frame_num)):
            ...
            for pid in range(person_num):
                ...
                joint_cam = np.dot(Rs[iid].reshape(3, 3), joint_3d[pid][iid].reshape(-1, 3).transpose(1, 0)).transpose(
                    1, 0
                ) + ts[iid].reshape(-1, 3)
                joint_img = cam2pixel(joint_cam, focal, princpt)

                # merge camera parameter with smpl parameter (rotation)
                ...
                annot_dict["smpl_param"] = {
                    "shape": shape.numpy().reshape(-1).tolist(),

...
# SMPl mesh
vertex_num = 6890
smpl_layer = SMPL_Layer(gender="neutral", model_root="smplpytorch/smplpytorch/native/models")
...
# img_path = "demo_output/input.jpg"
img_path = "demo_output/msasl_test_first_frame.png"
...
mesh_lixel_img[:, :2] = np.dot(
    bb2img_trans, np.concatenate((mesh_lixel_img[:, :2], np.ones_like(mesh_lixel_img[:, :1])), 1).transpose(1, 0)
).transpose(1, 0)
...
root_depth = (
    ...
    4500.22929688  # first frame 11250.5732421875/2.5
)
...
cv2.imwrite("demo/msasl_output_mesh_lixel.jpg", vis_img)

# visualize lixel mesh in 2D space
vis_img = original_img.copy()
mesh_param_img = cam2pixel(mesh_param_cam, focal, princpt)
vis_img = vis_mesh(vis_img, mesh_param_img)
cv2.imwrite("demo/msasl_output_mesh_param.jpg", vis_img)

# save mesh (obj)
save_obj(mesh_lixel_cam, face, "demo/msasl_output_mesh_lixel.obj")
save_obj(mesh_param_cam, face, "demo/msasl_output_mesh_param.obj")

# render mesh from lixel
vis_img = original_img.copy()
rendered_img = render_mesh(vis_img, mesh_lixel_cam, face, {"focal": focal, "princpt": princpt})
cv2.imwrite("demo/msasl_rendered_mesh_lixel.jpg", rendered_img)

# render mesh from param
vis_img = original_img.copy()
rendered_img = render_mesh(vis_img, mesh_param_cam, face, {"focal": focal, "princpt": princpt})
cv2.imwrite("demo/msasl_rendered_mesh_param.jpg", rendered_img)
